...
const Clients = ({ title }) => {
  return (
    <section id="clients" className="py-20 backdrop-blur-sm overflow-hidden">
      ...
      <div className="relative w-full bg-gray-800 py-14">
        <div className="flex gap-5 animate-scroll whitespace-nowrap px-6">
          {clients.concat(clients).map((client, index) => (
            <span key={index} className="inline-block px-6 py-2 bg-gray-900 text-white rounded-lg text-sm md:text-[16px] font-medium shadow hover:bg-blue-600 transition">
              {client}
            </span>
          ))}
        </div>
